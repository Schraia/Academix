...
<head>
    ...
    <title>Edit Note - Academix</title>
...
<div class="wrap">
    <div class="header">
        <div>
            ...
            <div class="subtitle">Update your note and tags. Downloads are available on the list page.</div>
        </div>
    ...
    <div class="card">
        <form method="POST" action="{{ route('notes.update', $note) }}">
            ...
            <div class="form-row">
                ...
                <div>
                    ...
                    @php $selected = collect(old('course_ids', $note->courses->pluck('id')->all())); @endphp
                    <select id="course_ids" class="select" name="course_ids[]" multiple>
                        @foreach($courses as $c)
                            <option value="{{ $c->id }}" {{ $selected->contains($c->id) ? 'selected' : '' }}>
        ...
        <form method="POST" action="{{ route('notes.destroy', $note) }}" id="delete-note-form" style="display:none;">
            @csrf
            @method('DELETE')
        </form>
    </div>
...
<script>
    (function () {
        ...
        function start(e) { drawing = true; last = canvasPoint(e); }
        function move(e) {
