    ...
    <div class="form-card">
        <form action="{{ route('courses.lessons.update', [$course, $lesson]) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <p class="hint" style="font-size: 0.875rem; color: #6b7280; margin-top: 0.25rem;">PDF, PPTX, DOCX, PNG, or video. Max 50MB.</p>
                @error('attachment') <p class="error">{{ $message }}</p> @enderror
            </div>
            <div class="form-group">
                <label for="video_url">YouTube link (optional)</label>
                <input type="url" id="video_url" name="video_url" value="{{ old('video_url', $lesson->video_url) }}" placeholder="https://www.youtube.com/watch?v=...">
